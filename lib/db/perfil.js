...
// Actualiza los datos editables del perfil (nombre/apellido/areaId) de un usuario
// ya autenticado. Se usa desde la pantalla "Editar Perfil".
export async function actualizarPerfil({ id, nombre, apellido, areaId }) {
  try {
    const profile = await prisma.profile.update({
      where: { id },
      data: { nombre, apellido, areaId: areaId || null },
    });

    // El perfil se cacheó al leerse: invalidamos su entrada para que el nuevo
    // nombre/apellido/area se vea de inmediato sin esperar al TTL.
