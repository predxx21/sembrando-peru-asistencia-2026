...
// primera carga). Ahora TODO se resuelve en UNA sola query ($queryRaw) con dos
// subconsultas agregadas que devuelven columnas JSON: agregados por perfil
// (JOIN a profiles para el nombre, sin query extra) y horas por día de la
// semana actual. El resumen global (pendientes, totalHoras, horasAprobadas,
// voluntariosActivos) usa la función compartida `obtenerResumenGlobal` de
// agregaciones.js (M-1). Así los 4 cards de métricas muestran valores idénticos
// en ambos paneles (/administracion y /reportes).
//
...
export async function obtenerReportes(areaId) {
  try {
    ...
    // Construir cláusula WHERE condicional para filtrar por área
    let whereArea = Prisma.empty;
    if (areaId) {
      whereArea = Prisma.sql`AND ra."profileId" IN (SELECT id FROM profiles WHERE "areaId" = ${areaId})`;
    }
    ...
    // ocurre en Postgres en un único round-trip. El resumen global se obtiene
    // vía obtenerResumenGlobal() (función compartida en agregaciones.js).
    const [filas, resumenGlobal] = await Promise.all([
      prisma.$queryRaw`
        SELECT
          (
      ...
      `,
      obtenerResumenGlobal(areaId),
    ]);

    const perfiles = parseJson(filas[0]?.perfiles) || [];
    const diasAgregados = parseJson(filas[0]?.dias) || [];

    const pendientes = resumenGlobal?.pendientes || 0;
    const totalHoras = resumenGlobal?.totalHoras || 0;
    const horasAprobadas = resumenGlobal?.horasAprobadas || 0;
    const voluntariosActivos = resumenGlobal?.voluntariosActivos || 0;
