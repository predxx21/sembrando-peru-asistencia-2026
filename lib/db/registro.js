...
function construirWhere({ profileId, estado, busqueda, desde, hasta, area }) {
  ...
  // Filtro por área (validar que exista en la lista de áreas permitidas)
  if (area) {
    const areasPermitidas = ['comedor', 'educación', 'salud', 'administración']; // ajustar según tu sistema
    if (!areasPermitidas.includes(area)) {
      // Si el área no es válida, ignorar el filtro (o lanzar error)
      // Opcional: devolver where sin área
    } else {
      where.profile = {
        ...where.profile,
        area: { equals: area },
