...
// M-2: select mínimo para listados (evita traer todas las columnas de
// profile y revisor). Solo lo que usa el frontend: nombres y área.
...
export async function guardarRegistroAsistencia({
  ...
  try {
    ...
    const registro = await prisma.registroAsistencia.create({
      data: {
        profileId,
        // A-3: fecha se guarda como medianoche UTC
        ...
        // C-1: ahora default es 'pendiente', pero lo dejamos explícito por claridad
        estado: 'pendiente',
      },
    });
    return { data: registro, error: null };
...
export async function corregirRegistro({
  ...
  try {
    // Validar hora fin > hora inicio (acepta cruce medianoche)
    if (!esHoraFinMayorAInicio(horaInicio, horaFin)) {
      return { data: null, error: { message: 'La hora de fin debe ser posterior a la de inicio.', status: 400 } };
    }
    ...
    // A-3: fecha se guarda como medianoche UTC (new Date("YYYY-MM-DD") → 00:00 UTC)
    const registro = await prisma.registroAsistencia.update({
      where: {
        id: Number(id),
