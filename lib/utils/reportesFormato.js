...
export function mapReportes(payload) {
  ...
  const stats = {
    ...
    promedioHoras: datos.stats?.promedioHoras ?? 0,
    meta: datos.stats?.meta ?? 15000,
    percent: datos.stats?.percent ?? 0,
  };

  const porMes = (datos.porMes || []).map(({ month, year, value }) => ({
    month,
    year,
  ...
  return { stats, porMes, contribuyentes, voluntarios };
}
