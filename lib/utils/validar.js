...
// ¿La evidencia pertenece al usuario (está en su carpeta privada)?
// Formato esperado: "<userId>/<archivo>". Solo permite rutas dentro de la
// carpeta del usuario para prevenir IDOR (Insecure Direct Object Reference).
export function esEvidenciaPropia(ruta, userId) {
  if (typeof ruta !== 'string' || typeof userId !== 'string') return false;
  const esperado = `${userId}/`;
  return ruta.startsWith(esperado);
}
