...
describe('mapReportes', () => {
  ...
  it('deja ultimaActividadISO en null cuando el voluntario no tiene actividad', () => {
    const payload = {
      ...PAYLOAD,
      voluntarios: [
        { profileId: 'u9', nombre: 'Sin', apellido: 'Actividad', registros: 0, horas: 0 },
      ],
    };

    const res = mapReportes(payload);

    expect(res.voluntarios[0].ultimaActividad).toBe('—');
    expect(res.voluntarios[0].ultimaActividadISO).toBeNull();
  });
});
